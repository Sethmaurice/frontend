...
const Houses = ({ url }) => {
  ...
  const currentItems = filteredByAvailability.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  return (
    <div className="house__homepage">
      ...
      <div className="houses__header">
        <input
          type="text"
          className="login__input inputs"
          placeholder="search by location"
          value={searchLocation}
          onChange={handleLocationChange}
          onKeyDown={() => filteredByLocation}
        />
        <button className="header__btn" onClick={() => filterByStatus("rent")}>
          Rent
        </button>
        <button className="header__btn" onClick={() => filterByStatus("buy")}>
          Sell
        </button>
      </div>
      ...
      <div className="pagination">
        ...
        <button
          className="header__btn"
          onClick={() => paginate(currentPage + 1)}
          disabled={indexOfLastItem >= filteredByAvailability.length}
        >
          Next
        </button>
      </div>
